...
const pintarBoleto = () => {
    ...
    Object.values(boleto).forEach(elementos => {
        ...
        <td>${elementos.expOrdinaria}</td>
        <td>${elementos.expExtraordinaria}</td>
        <td>${elementos.deuda ? "3500" : "null"}</td>
        <td>$ <span>500</span></td>
      </tr> 
          `

       
     /*  templateBoleto.querySelector('th').textContent = elementos.id
        templateBoleto.querySelectorAll('td')[0].textContent = elementos.nombre
        templateBoleto.querySelectorAll('td')[1].textContent = elementos.expOrdinaria
        templateBoleto.querySelectorAll('td')[2].textContent = elementos.expExtraordinaria
        templateBoleto.querySelectorAll('td')[3].textContent = (elementos.deuda ? "3500")

     
        const clone = templateBoleto.cloneNode(true)
        fragment.appendChild(clone)


    } )
  boletos.appendChild(fragment)
